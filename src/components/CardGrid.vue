<template>
    <v-container v-if="items.length || loading" ref="containerRef" fluid class="pa-0">
        <div class="virtual-spacer" :style="{ height: `${virtualizer.getTotalSize()}px` }">
            <v-row v-for="virtualRow in virtualizer.getVirtualItems()" :key="virtualRow.index" dense
...
import { useWindowVirtualizer, windowScroll } from "@tanstack/vue-virtual";
...
const virtualizer = useWindowVirtualizer<HTMLElement>(computed(() => ({
    ...
    // The library's default behavior nudges the WHOLE window's scroll to
    // compensate whenever a row's real size first differs from the
    // estimate — meant to keep content visually stable for a viewer already
    // scrolled into a full-page list. This grid is usually one piece of a
    // page among others, so "this grid's first measurement" is not a reason
    // to move a scroll position that may belong to unrelated content above
    // or below it (e.g. it fires on page load, at scrollY 0, for any grid
    // that isn't the very first thing on the page). The documented escape
    // hatch for this (`shouldAdjustScrollPositionOnItemSizeChange`) isn't
    // actually wired up to anything in the installed version, so filter it
    // out at the one place it's guaranteed to funnel through instead:
    // `applyScrollAdjustment` is the only caller that passes a defined
    // `adjustments` value — every real scroll (ours or the library's own
    // mount/reconcile sync) leaves it undefined.
    scrollToFn: (offset, opts, instance) => {
        if (opts.adjustments !== undefined) return;
