...
describe("BaseMenu", () => {
    it("keeps its slot content mounted even before the menu is ever opened (eager)", () => {
        ...
        // ButtonAddToPlaylist) gets destroyed - dialog included - the instant the menu
        // auto-closes on that same click (close-on-content-click defaults to true), so
        // the dialog flashes and disappears before the user can use it. `eager` keeps
        // the content mounted throughout, so a nested dialog survives.
